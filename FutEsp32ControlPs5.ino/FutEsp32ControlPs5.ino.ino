...
void procesarEntradaMando(GamepadPtr gp) {
    ultimoMensaje = millis(); // Resetear temporizador de seguridad

    if (gp->b()) {
        saludo();
    }

    int32_t gatilloR2 = gp->throttle(); // Valores de 0 a 1023
    int32_t gatilloL2 = gp->brake();    // Valores de 0 a 1023
    ...
    velocidadMotorA = avance + giro; // Llantas del lado izquierdo
    velocidadMotorB = avance - giro; // Llantas del lado derecho
}
void setup() {
    pinMode(PWMA, OUTPUT);
    pinMode(PWMB, OUTPUT);
